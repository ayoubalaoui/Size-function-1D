#include "GenericSplineCurve.h"

GenericSplineCurve::GenericSplineCurve(const vector<Vec<4, float>>& _which)
{
	this->plist.clear();
...
GenericSplineCurve::GenericSplineCurve(const GenericSplineCurve& Q) // non testé
{
...
void GenericSplineCurve::destroy() {
	curve_point.clear();
	plist.clear();
}
void GenericSplineCurve::compute()
{
	curve_point.clear();
	this->curve_point.reserve(this->precision + 1);
	...
	for (int i = 0; i < this->nb_curve_point; i++)
	{
		...
		for (int j = 1; j <= this->nb_point; j++) // sommer tous les points est très lourd (puisque pour la plupart, spline_uniform(...) retourne 0), mais tellement plus simple. A optimiser donc...
		{
			...
			tmp += tb * this->plist[j - 1];
		}
		tb = 0;
		u += p;
		//this->curve_point[i][0] = tx;
		//this->curve_point[i][1] = ty;
		curve_point.push_back(Vec<4,float>(tmp));
	}


}
float GenericSplineCurve::spline_uniform(float t, float x) // fonction allure Gauss, maximum en x = t
{
	t -= 2.0f;
