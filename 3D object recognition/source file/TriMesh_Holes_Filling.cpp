...
#define VERT(v) (((v) < themesh->vertices.size()) ? \
		themesh->vertices[v] : \
		newverts[(v) - themesh->vertices.size()].p)
#define NORM(v) (((v) < themesh->vertices.size()) ? \
		themesh->normals[v] : \
		newverts[(v) - themesh->vertices.size()].norm)
...
void holes_filling(TriMesh *themesh){
	bool fillonly = false;
	bool listonly = false;
	bool nocurv = false;
	int maxfill = 0;
	...
	vector<VertStruct> newverts;
	for (int i = 0; i < holes->size(); i++) {
		if (maxfill && (*holes)[i].size() > maxfill)
			continue;
		printf("\r                                                                             \r");
		printf("Filling hole %d of %d, size %d: ", i+1, (int)holes->size(), (int)(*holes)[i].size()); fflush(stdout);
